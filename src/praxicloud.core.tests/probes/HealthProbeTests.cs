...
namespace praxicloud.core.tests.exceptions
{
    ...
    public class HealthProbeTests
    {
        ...
        public void StartStop()
        {
            ...
            var probe = new TestHealthProbe(IPAddress.Any, 10034, TimeSpan.FromMilliseconds(1), checker);
    ...
    #region TestHealthProbe Definition
    /// <summary>
    /// A health probe that counts invocations of specialized events
    /// </summary>
    public class TestHealthProbe : HealthContainerProbe
    {
        #region Variables
        /// <summary>
        ...
        /// <param name="handler">A handler used to determine if the service is healthy</param>
        public TestHealthProbe(ushort port, TimeSpan pollingInterval, IHealthCheck handler) : base(port, pollingInterval, handler)
        {
        ...
        /// <param name="pollingInterval">The interval between checking for new connection requests</param>
        /// <param name="handler">A handler used to determine if the service is healthy</param>
        public TestHealthProbe(IPAddress address, ushort port, TimeSpan pollingInterval, IHealthCheck handler) : base(address, port, pollingInterval, handler)
        {
    ...
    /// <summary>
    /// A test health check that the consumer controls the value of
    /// </summary>
    public class TestHealthCheck : IHealthCheck
    {
        #region Properties
        /// <summary>
        /// Set by the consumer to indicate the health response
        /// </summary>
        ...
        {
            return Task.FromResult(CurrentHealthy);
        }
        #endregion
    }
    #endregion
